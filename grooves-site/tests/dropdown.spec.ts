...
test.describe('Dropdown Functionality Tests', () => {
  ...
  test('should show dropdown when clicking Documentation button', async ({ page }) => {
  ...
  test('should populate dropdown with version data from versions.json', async ({ page }) => {
    const docButton = page.locator('#dropdownMenuLink');
    await docButton.click();
    ...
    await expect(dropdown.locator('.dropdown-header').filter({ hasText: 'Upcoming' })).toBeVisible();
    await expect(dropdown.locator('a[href*="0.8.0-SNAPSHOT"]')).toBeVisible();
    
    // Check for old versions section
    await expect(dropdown.locator('.dropdown-header').filter({ hasText: 'Old' })).toBeVisible();
  ...
  test('should have proper dropdown structure with headers and dividers', async ({ page }) => {
    const docButton = page.locator('#dropdownMenuLink');
    await docButton.click();
  ...
  test('should have correct href patterns for documentation links', async ({ page }) => {
    const docButton = page.locator('#dropdownMenuLink');
    await docButton.click();
  ...
  test('should load versions.json successfully', async ({ page }) => {
    // Monitor network requests
    let versionsLoaded = false;
    page.on('response', response => {
      if (response.url().includes('versions.json') && response.ok()) {
        versionsLoaded = true;
      }
    });
    ...
    expect(versionsLoaded).toBe(true);
  });
});
