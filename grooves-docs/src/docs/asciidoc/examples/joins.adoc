...
<2> This says that the aggregate for which this computes joins is `Doctor` whose id is `Long`.
<3> This says that the base class for events for which this computes joins is `DoctorEvent` whose id is `Long`.
<4> This says that the joined aggregate for which this computes joins is `Patient` whose id is `Long`.
