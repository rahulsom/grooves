...
.Java
----
include::{sourcedir}/examples/grooves-example-javaee/src/main/java/grooves/example/javaee/domain/PatientAccount.java[tags=documented]
----
...
.Groovy
----
include::{sourcedir}/examples/grooves-example-springboot/grooves-example-springboot-jpa/src/main/groovy/grooves/boot/jpa/domain/PatientAccount.groovy[tags=documented]
----
<1> For most snapshots, you may implement `Snapshot`.
<2> The `lastEventPosition` is useful when locating snapshots by version.
<3> The `lastEventTimestamp` is useful when locating snapshots by timestamp.
...
.Kotlin
----
include::{sourcedir}/examples/grooves-example-springboot/grooves-example-springboot-kotlin/src/main/kotlin/grooves/boot/kotlin/domain/PatientAccount.kt[tags=documented]
----
<1> For most snapshots, you may implement `Snapshot`.
<2> The `lastEventPosition` is useful when locating snapshots by version.
<3> The `lastEventTimestamp` is useful when locating snapshots by timestamp.
