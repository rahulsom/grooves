...
class ProcedurePerformed extends PatientEvent {
    ...
    @Override String toString() {
        "ProcedurePerformed(code=$code, cost=$cost)"
    }
}
...
implements RevertEvent<Patient, Long, PatientEvent> {
    ...
    @Override String toString() {
        "PatientEventReverted(revertedEventId=$revertedEventId)"
    }
}
// end::reverted[]
...
DeprecatedBy<Patient, Long, PatientEvent> {
    ...
    Publisher<PatientDeprecates> getConverseObservable() {
        just(converse)
    }
    Publisher<Patient> getDeprecatorObservable() {
        just(deprecator)
    }

    @Override String toString() {
        "PatientDeprecatedBy(deprecator=$deprecator)"
    }
}
...
implements Deprecates<Patient, Long, PatientEvent> {
    ...
    Publisher<PatientDeprecatedBy> getConverseObservable() {
        just(converse)
    }
    Publisher<Patient> getDeprecatedObservable() {
        just(deprecated)
    }

    @Override String toString() {
        "PatientDeprecates(deprecated=$deprecated)"
    }
}
...
JoinEvent<Patient, Long, PatientEvent, Zipcode> {
    ...
    @Override Publisher<Zipcode> getJoinAggregateObservable() {
        just(zipcode)
    }
