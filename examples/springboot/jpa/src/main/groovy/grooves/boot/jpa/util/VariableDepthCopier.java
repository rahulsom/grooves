 ...
 *
 * <p>If level is set to zero, non-primitive properties will be set to null and collections will
 * be empty.
 * This copy performs cycle detection and sets any repeated copies to null.
 *
 * <p>If a property is of type byte[], the copy is a direct reference to the original array, no
 * array copy is done.
 *
 * <p>This works for Hibernate objects because hibernate's persistent collections aren't copied,
 * their contents are copied into the default collection of the target class.
 *
 * <p>Copy is done according to bean properties, not necessarily through field reflection.
 *
 * <p>This class has no mutable state so is thread-safe.
 */
        ...
        try {
            ...
            if (depth > 0) {
                ...
                PropertyDescriptor[] descriptors =
                        BeanUtils.getPropertyDescriptors(from.getClass());
                for (PropertyDescriptor pd : descriptors) {

                    // copy into collections (target should always have a default empty collection
                    // assigned)
                    ...
                    if (nonSimplePropertyNames.contains(pd.getName())) {
                        ...
                        Object propertyCopy = createObjectCopy(
                                propertyToCopy, depth - 1, ancestors);
