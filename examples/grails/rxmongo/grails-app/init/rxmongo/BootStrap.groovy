...
class BootStrap {
    ...
    private Patient setupJohnLennon() {
        def patient = new Patient(uniqueId: '42').save(flush: true, failOnError: true).toBlocking().first() as Patient
    ...
    private Patient setupRingoStarr() {
        def patient = new Patient(uniqueId: '43').save(flush: true, failOnError: true).toBlocking().first() as Patient
    ...
    private Patient setupPaulMcCartney() {
        def patient = new Patient(uniqueId: '44').save(flush: true, failOnError: true).toBlocking().first() as Patient
    ...
    private Patient setupGeorgeHarrison() {
        def patient = new Patient(uniqueId: '45').save(flush: true, failOnError: true).toBlocking().first() as Patient
        def patient2 = new Patient(uniqueId: '46').save(flush: true, failOnError: true).toBlocking().first() as Patient
    ...
    Patient on(Patient patient, @DelegatesTo(EventsDsl.OnSpec) Closure closure) {
        def eventSaver = { it.save(flush: true, failOnError: true).toBlocking().first() } as Consumer<PatientEvent>
