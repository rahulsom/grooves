...
class PatientTest extends AbstractPatientTest {
    ...
    public static void setupSpec() {
        var warFile = new File(
                System.getProperty("war.file.path", "build/libs/grooves-example-javaee-0.1.war"));
        if (!warFile.exists()) {
            throw new IllegalStateException(
                    "WAR file not found at " + warFile.getAbsolutePath()
                    + ". Run './gradlew war' first.");
        }
