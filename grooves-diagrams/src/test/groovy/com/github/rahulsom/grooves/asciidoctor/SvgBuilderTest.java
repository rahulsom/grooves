...
public class SvgBuilderTest {
    @ParameterizedTest
    @ValueSource(strings = {"SimpleEvents", "RevertEvent", "RevertEventEffective", "RevertOnRevert", "RevertOnRevertEffective", "MergeAggregates", "MergeAggregatesEffective", "RevertMergeBefore", "RevertMergeAfter", "JoinExample", "DisjoinExample", "ParallelEvents", "ConcurrentEvents"})
    public void test(final String name) throws IOException {
        ...
        if (!new File("src/test/resources/" + name + ".esdiag.svg").exists()) {
            Files.write(new File("src/test/resources/" + name + ".esdiag.svg").toPath(), Files.readAllBytes(file.toPath()));
        }
